...
const D3BarChart: React.FC<D3BarChartProps> = ({
   ...
   // Applying DNV colors
   barColor = '#007C92', // DNV Blue
   barHoverColor = '#5CB02C', // DNV Green
   axisColor = 'rgba(105, 105, 105, 0.8)', // DNV Gray with opacity for axis
   tooltipBackgroundColor = '#007C92', // DNV Blue for tooltips
   tooltipTextColor = 'white',
   gridColor = 'rgba(105, 105, 105, 0.1)', // DNV Gray for gridlines
   labelColor = 'rgba(105, 105, 105, 0.8)', // DNV Gray for labels
   xAxisTitle,
   yAxisTitle,
}) => {
   ...
   const createChart = (width: number, height: number) => {
      if (svgRef.current) {
         ...
         const y = d3
            .scaleLinear()
            .domain([0, d3.max(data, (d) => d.value) || 0]) // Input domain (0 to max value of the data)
         ...
         g.append('g')
            .call(d3.axisLeft(y).tickSize(-innerWidth).tickPadding(10))
         ...
         g.selectAll('.bar')
            ...
            .on('mouseover', function (event, d) {
               ...
               tooltip
                  .html(`<div class="tooltip-content">Value: <span class="tooltip-value">${d.value}</span></div>`)
